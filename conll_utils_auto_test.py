...
def get_tree_data(raw_data, character_to_index, word_to_index, pos_to_index, auto_data):
    log("get_tree_data()...")
    ...
    for document in raw_data:
        for part in raw_data[document]:
            ...
            feature_data = auto_data if auto_data else raw_data
            
            for index, parse in enumerate(feature_data[document][part]["parses"]):
                head_raw_data = feature_data[document][part]["heads"][index]
                text_raw_data = feature_data[document][part]["text"][index]
...
def read_conll_dataset(raw_data_path = "../CONLL2012-intern/conll-2012/v4/data",
    ...
    # Read auto data for test
    auto_data = {}
    full_path = os.path.join("/home/danniel/Downloads/wu_conll_test/v9/data/test/data/english/annotations")
    config = {"file_suffix": "auto_conll", "dir_prefix": full_path}
    auto_data["test"] = load_data(config)
    ...
    for split in data_split_list:
        ...
        tree_data, degree, pos_count[split], ne_count[split], pos_ne_count[split], ner_list = (
            get_tree_data(raw_data[split], character_to_index, word_to_index, pos_to_index, auto_data[split]))
