...
def train():
    # Read data
    data, word_to_index, labels, pos_dimension, characters, ne_list = (
...
def make_tree_ner_batch(tree_list, ner_list, span_set_list):
    data = zip(tree_list, ner_list, span_set_list)
    data = sorted(data, key=lambda x: x[0].nodes)
    
    batch_list = []
    batch = []
    for tree, ner, span_set in data:
        if len(batch)>=batch_trees or (len(batch)+1)*tree.nodes>batch_nodes:
            batch_list.append(batch)
            batch = []
        batch.append((tree, ner, span_set))
    batch_list.append(batch)
...
def evaluate_dataset(model, data, ne_list):
    tree_list, ner_list, span_set_list = data
    batch_list = make_tree_ner_batch(tree_list, ner_list, span_set_list)
    # print "batches:", len(batch_list)
    ...
    for batch in batch_list:
        tree_list, ner_list, span_set_list = zip(*batch)
        true_postives, postives, reals = model.evaluate(tree_list, ner_list, span_set_list, ne_list)
